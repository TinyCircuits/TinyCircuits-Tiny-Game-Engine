...
mp_obj_t circle_2d_node_class_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args){
    ...
    node_base_init(node_base, NULL, &engine_circle_2d_node_class_type, NODE_TYPE_CIRCLE_2D);
    ...
    if(inherited == true){  // Inherited (use existing object)
        ...
        MP_OBJ_TYPE_SET_SLOT((mp_obj_type_t*)((mp_obj_base_t*)node_instance)->type, attr, circle_2d_node_class_attr, 5);
