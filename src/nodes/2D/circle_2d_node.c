...
static mp_attr_fun_t circle_2d_node_class_attr(mp_obj_t self_in, qstr attribute, mp_obj_t *destination){
    ...
    node_base_attr_handler(self_in, attribute, destination,
                          (attr_handler_func[]){node_base_load_attr, circle_2d_node_load_attr},
                          (attr_handler_func[]){node_base_store_attr, circle_2d_node_store_attr}, 2);
...
mp_obj_t circle_2d_node_class_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args){
    ...
    node_base_init(node_base, &engine_circle_2d_node_class_type, NODE_TYPE_CIRCLE_2D, parsed_args[layer].u_int);
