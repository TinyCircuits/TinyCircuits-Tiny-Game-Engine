...
typedef struct{
    ...
    uint32_t source_sample_index;               // The total index position inside the source that the ISR is using to start filling from
    ...
    uint16_t buffer_sample_index;               // Where we are in the buffer pointed to by 'fill_buffer_index'. Loop back to 0 when reach sample count stored in 'source'
