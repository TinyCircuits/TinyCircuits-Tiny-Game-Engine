...
audio_channel_class_obj_t *engine_audio_get_channel(uint8_t channel_index){
    return (audio_channel_class_obj_t*)channels[channel_index];
}
...
void engine_audio_setup_playback(){
    #if defined(__EMSCRIPTEN__)
        engine_audio_web_setup_playback();
    #elif defined(__unix__)
        engine_audio_unix_setup_playback();
    #elif defined(__arm__)
        engine_audio_rp3_setup_playback();
    #endif
}
...
void engine_audio_init_one_time(){
    ...
    // before any callbacks tries to access the channels array
