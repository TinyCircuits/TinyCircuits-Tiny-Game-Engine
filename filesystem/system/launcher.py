...
def get_games_in_dir(dir=None):
    """Returns the GameInfo or GameListInfo representing the game(s) in the given dir."""
    if dir:
        game = get_game_in_dir(dir)
        if game:
            return game
    gli = GameListInfo(basename(dir) if dir else None)
    for entry in os.listdir(dir or "."):
        subdir = f"{dir}/{entry}" if dir else entry
        if is_dir(subdir):
...
def create_tile(game_info):
    global index
    tile = LauncherTile(game_info)
