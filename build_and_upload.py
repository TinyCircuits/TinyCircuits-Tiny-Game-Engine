...
#     output = ""
#     process = subprocess.Popen("sudo python3 run.py " + clean_or_path_arg + "/" + file, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
#     # with subprocess.Popen(['sudo', 'python3', 'run.py', clean_or_path_arg + "/" + file], stdout=PIPE, bufsize=1, universal_newlines=True) as p:
#     #     for line in p.stdout:
#     #         print(line, end='') # process line here
#     #         output += line

#     # process = subprocess.run(['sudo', 'python3', 'run.py', clean_or_path_arg + "/" + file], shell=True, capture_output=True, text=True)
